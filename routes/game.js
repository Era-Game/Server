...
//         users.forEach((user) => {
//             if (user.val() === _uid) {
//                 isInTeam = true; // 防止 userIsTeam is false
//             }
//             if (user.key == "0") { //leader index=0
//                 leader_uid = user.val();
//             }
//         });
//
...
//
// /*
//     save personal best time (record each user's record in every team)
//     one user might run more than twice
//  */
...
//
// /*
//     User gets game status.
//     Get game info: winner_teamID ...
//
//     P.S. fetch other team's runner data(current runner, runner skin) by using Unity Firebase SDK
//  */
...
//         });
//
//         for (let i = _teamNameList.length; i < 5; i += 1) { // maximum team number is 5
...
// // ----------------------------- AFK Handler ----------------------------------------------------
// /*
//     do transition while current runner is offline
//  */
...
//     (Last runner upload the winner data.)
//  */
...
//
//         if (isWinner) {
//             return snap.ref.parent.parent.parent.child("anyWin").set(win); // game -> anyWin
//         }
//
//         return null;
//     });
...
// /*
//     calling: team update status
//     target: check each team status and set start game flag
//  */
...
//                     startGame: true,
//                 };
//                 return change.after.ref.parent.parent.parent.update(updates); // update flag (game -> .... -> startGame)
//             }
//
//             return null;
//         });
//     });
//
// // relayRunnerTransition
// /*
//     1. check team status (isDead or not)
//     2. runner transition (check current accumulated distance)
//  */
...
//         }
//
//         let relay_steps_threshold = 0; // game var (in DB)
//         let relay_totalsteps = 0; // game var (in DB)
...
//
//         database.ref("Config_Constants/").once("value").then((config) => {
//             config.forEach((constants) => { // fetch variable
//                 if (constants.key === "RELAY_STEPS_THRESHOLD") {
//                     relay_steps_threshold = parseInt(constants.val(), 10);
//                 }
//                 if (constants.key === "RELAY_TOTALSTEPS") {
...
//
//             /*
//                 User distance > threshold(200, 150, 50) => invoke transaction()
//              */
...
//
//                 // checkpoint,confirm only do one time transition (Share variable) (WS)
...
// /*
//     Caller: every modification in team info (team match, game(AFK))
//  */
...
// /*
//     Caller: every modification in game info (game(AFK))
//  */
